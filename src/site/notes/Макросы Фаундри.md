---
{"tags":["foundry"],"dg-publish":true,"aliases":["Macros Foundry"],"permalink":"/makrosy-faundri/","dgPassFrontmatter":true}
---
...
async function tokenUpdate(lightData) {
  ...
  for (const token of canvas.tokens.controlled) {
    ...
    const existingEffect = token.actor.effects.find(e => e.img === TORCH_ICON);

    if (turnedOn && !existingEffect) {
      await ActiveEffect.implementation.create({
        name: "Torch Lit",
        img: TORCH_ICON,
        origin: token.actor.uuid,
        "flags.core.statusId": "torch-lit",
        duration: {
          rounds: 600,   // Exact DnD 5e Duration (1 hour of combat rounds)
          seconds: 3600  // Exact DnD 5e Duration (1 hour in seconds)
        }
      }, { parent: token.actor });
    }
    else if (!turnedOn && existingEffect) {
...
const dialogConfig = {
  ...
  content: `
    <p style="text-align: center; margin-bottom: 14px; font-weight: bold;">
      Pick the light source the selected token is holding:
    </p>
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px 20px; margin-bottom: 18px; padding: 0 10px;">
      
      <div style="display: flex; flex-direction: column;">
        <label style="display: flex; align-items: center; gap: 6px; cursor: pointer; font-weight: bold;">
          <input type="radio" name="lightSource" value="none" checked> None
        </label>
        <small style="padding-left: 22px; opacity: 0.7;">Extinguish all held light sources.</small>
      </div>

      <div style="display: flex; flex-direction: column;">
        <label style="display: flex; align-items: center; gap: 6px; cursor: pointer; font-weight: bold;">
          <input type="radio" name="lightSource" value="torch"> Torch <a href="https://www.dndbeyond.com/equipment/437-torch" target="_blank" style="margin-left: 4px; color: #ff9f43;"><i class="fas fa-book-open"></i></a>
        </label>
        <small style="padding-left: 22px; opacity: 0.7;">
          20ft Bright / 40ft Dim (1 hour) 
        </small>
      </div>

      <div style="display: flex; flex-direction: column;">
        <label style="display: flex; align-items: center; gap: 6px; cursor: pointer; font-weight: bold;">
          <input type="radio" name="lightSource" value="candle"> Candle <a href="https://www.dndbeyond.com/equipment/501-candle" target="_blank" style="margin-left: 4px; color: #ff9f43;"><i class="fas fa-book-open"></i></a>
        </label>
        <small style="padding-left: 22px; opacity: 0.7;">
          5ft Bright / 5ft Dim (1 hour)
        </small>
      </div>

      <div style="display: flex; flex-direction: column;">
        <label style="display: flex; align-items: center; gap: 6px; cursor: pointer; font-weight: bold;">
          <input type="radio" name="lightSource" value="bullseye"> Bullseye Lantern <a href="https://www.dndbeyond.com/equipment/494-bullseye-lantern" target="_blank" style="margin-left: 4px; color: #ff9f43;"><i class="fas fa-book-open"></i></a>
        </label>
        <small style="padding-left: 22px; opacity: 0.7;">
          60ft B / 120ft D (45° Cone, 6 hrs) 
        </small>
      </div>

      <div style="display: flex; flex-direction: column;">
        <label style="display: flex; align-items: center; gap: 6px; cursor: pointer; font-weight: bold;">
          <input type="radio" name="lightSource" value="hooded"> Hooded Lantern <a href="https://www.dndbeyond.com/equipment/493-hooded-lantern" target="_blank" style="margin-left: 4px; color: #ff9f43;"><i class="fas fa-book-open"></i></a>
        </label>
        <small style="padding-left: 22px; opacity: 0.7;">
          30ft Bright / 60ft Dim (6 hours) 
        </small>
      </div>

      <div style="display: flex; flex-direction: column;">
        <label style="display: flex; align-items: center; gap: 6px; cursor: pointer; font-weight: bold;">
          <input type="radio" name="lightSource" value="light"> Light <a href="https://www.dndbeyond.com/spells/2618996-light" target="_blank" style="margin-left: 4px; color: #a6bbdd;"><i class="fas fa-magic"></i></a>
        </label>
        <small style="padding-left: 22px; opacity: 0.7;">
          20ft Bright / 40ft Dim (1 hour) 
        </small>
      ...
      <div style="display: flex; flex-direction: column;">
        <label style="display: flex; align-items: center; gap: 6px; cursor: pointer; font-weight: bold;">
          <input type="radio" name="lightSource" value="darkness"> Darkness spell <a href="https://www.dndbeyond.com/spells/2058-darkness" target="_blank" style="margin-left: 4px; color: #a6bbdd;"><i class="fas fa-magic"></i></a>
        </label>
        <small style="padding-left: 22px; opacity: 0.7;">
          15ft Magical Darkness (10 min) 
        </small>
      </div>
  ...
  buttons: [
    {
      ...
      label: "Apply Light",
      default: true,
      // Extracts whichever radio choice is currently selected
      callback: (event, button, dialog) => {
        const choice = button.form.elements.lightSource.value;
        ...
        switch (choice) {
          case "none":
            ...
            break;
          case "torch":
            tokenUpdate({ ...mundaneLight, "bright": 20, "dim": 40, isTorch: true });
            break;
          case "light":
            tokenUpdate({ ...magicLight, "bright": 20, "dim": 40 });
            break;
          case "candle":
            tokenUpdate({ ...mundaneLight, "bright": 5, "dim": 10 });
            break;
          case "bullseye":
          ...
          case "hooded":
            ...
            break;
          case "darkness":
            tokenUpdate({ ...magicLight, "bright": 15, "dim": 0, "luminosity": -0.5, "negative": true });
            break;
        }
      }
    },
...
new foundry.applications.api.DialogV2(dialogConfig).render({ force: true });
```
## И цзин
```javascript
hexagrams = [
